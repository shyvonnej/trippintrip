...
if (isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO tbl_personas (name, persona_type, description, prefered_activity, image_url, created_at, updated_at) VALUES (:name, :type, :desc, :activity, :image, NOW(), NOW())");
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Successfully added new Persona')</script>";
        echo "<script>window.location.replace('personas.php')</script>";
    } else {
        echo "<script>alert('Failed to add Persona')</script>";
        echo "<script>window.location.replace('addpersonas.php')</script>";
    }
    ...
    <div class="w3-main w3-content w3-padding w3-center" style="max-width:1500px;margin-top:20px">
        <div style="display:flex; justify-content: center">
            <div class="w3-container w3-card w3-padding w3-margin" style="width:600px;margin:auto;text-align:left;">
                <h3 class="w3-center">Add New Persona</h3>
                <form action="addpersonas.php" method="POST">
                    <p style="display: flex; align-items: center;">
                        <label for="name" class="form-label" style="margin-top: 10px">Name:</label>
                        <input type="text" name="name" class="form-field" style="margin-top: 10px"><br>
                    </p>
                    <p style="display: flex; align-items: center;">
                        <label for="persona_type" class="form-label">Type:</label>
                        <input type="text" name="persona_type" class="form-field"><br>
                    </p>
                    <p style="display: flex; align-items: center;">
                    ...
                    <p style="display: flex; align-items: center;">
                        ...
                        <input type="text" name="prefered_activity" class="form-field"><br>
                    </p>
                    <p style="display: flex; align-items: center;">
                        <label for="image_url" class="form-label">Image URL:</label>
                        <input type="text" name="image_url" class="form-field"><br>
                    </p>
